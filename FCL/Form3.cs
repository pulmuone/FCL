...
namespace FCL
{
    public partial class Form3 : Form
    {
        ...
        /// <returns></returns>
        private byte[] Encoded_Image_Bytes(string Content)
        {
            byte[] data = null;
            Image Encoded_Image = null;
            ...
            try
            {
                ...
                Encoded_Image = writer.Write(Content);

                if (Encoded_Image == null) return null;

                using (MemoryStream ms = new MemoryStream())
                {
                    Encoded_Image.Save(ms, System.Drawing.Imaging.ImageFormat.Png);
                    data = ms.ToArray();
                }
            }
            catch (Exception ex)
